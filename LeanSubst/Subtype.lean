...
namespace LeanSubst
  ...
  namespace Subtype
    ...
    @[simp]
    theorem prj_I : prj P I = I := by
      have lem := prj_ren P (λ x => x); simp at lem
      apply lem

    @[simp]
    theorem prj_S : prj P S = S := by
    ...
    @[simp]
    def lift : Subst.Lift {t:T // P t} -> Subst.Lift T
    | lf, f => prj P $ lf (inj P f)
    ...
    omit [SubstMap T] in
    theorem lift_ren_lift :
      lift P (Ren.to ∘ Ren.lift ∘ Ren.fro) = Ren.to ∘ Ren.lift ∘ Ren.fro
    := by
      funext; case _ σ x =>
      cases x <;> simp [Ren.to, Ren.lift, Ren.fro]
      case _ x =>
      generalize zdef : σ x = z at *
      cases z <;> simp at *

    class SubstMapClosed {T : Type u} [SubstMap T] (P : T -> Prop) [SubtypeInject P] where
      closed : ∀ lf f t, P t -> P (SubstMap.smap (lift P lf) (prj P f) t)

    @[simp]
    def smap [i : SubstMapClosed P] :
      Subst.Lift {t:T // P t} ->
      (Nat -> Subst.Action {t:T // P t}) ->
      {t:T // P t} -> {t:T // P t}
    | lf, f, .mk t p => .mk (SubstMap.smap (lift P lf) (prj P f) t) (i.closed lf f t p)
