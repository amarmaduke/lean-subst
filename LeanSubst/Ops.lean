...
@[simp]
theorem Subst.id_action {x} : (id T).act x = re x := by simp [id, act]
----------------------------------------------------------------------------------------------------
---- Successor
----------------------------------------------------------------------------------------------------
...
@[simp]
theorem Subst.succ_action {x} : (succ T).act x = re (x + 1) := by simp [succ, act]
----------------------------------------------------------------------------------------------------
---- Predecessor
----------------------------------------------------------------------------------------------------
...
@[simp]
theorem Subst.pred_action {x} : (pred T).act x = re (x - 1) := by simp [pred, act]
----------------------------------------------------------------------------------------------------
---- Addition
----------------------------------------------------------------------------------------------------
...
@[simp]
theorem Subst.add_action {k x} : (add T k).act x = re (x + k) := by simp [add, act]
...
@[simp]
theorem Subst.sub_action {k x} : (@sub T k).act x = re (x - k) := by simp [sub, act]
...
@[simp]
theorem Subst.cons_action0 {a} {σ : Subst T} : (a::σ).act 0 = a := by simp [cons, act]

@[simp]
theorem Subst.cons_action {a i} {σ : Subst T} : (a::σ).act (i + 1) = σ.act i := by simp [cons, act]
----------------------------------------------------------------------------------------------------
---- Append
----------------------------------------------------------------------------------------------------
...
@[simp]
theorem Subst.act_list_nil {σ : Subst T} : σ.act ([] : List Nat) = [] := by simp [act_list, act]

@[simp]
theorem Subst.act_list_cons {σ : Subst T} {x} {ℓ : List Nat} : σ.act (x::ℓ) = σ.act x :: σ.act ℓ :=
  by simp [act_list, act]
...
@[simp]
theorem Ren.to_act {r : Ren T} {x} : (@to T r).act x = re (r.act x) := by simp [to, SubstAction.act]
...
theorem Ren.to_lift [RenMap T T] {r : Ren T} {k} : (r.lift k).to = (@to T r).lift k := by
  cases r; simp [to, lift, Subst.lift, SubstAction.act]; case _ act =>
  funext; case _ x =>
  cases x; grind
...
:= by
  funext; case _ x =>
  cases x <;> simp [to, compose, Subst.compose, SubstAction.act]
